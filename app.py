...
# Stałe z wzoru
CONSTANT_SIATKI_DEFAULT = 11697.80  # Dla innych gramatur
CONSTANT_SIATKI_200 = 12246.68     # Dla gramatury 200 g/m²
CONSTANT_FORMATKI = 13140.56

# Funkcja do obliczania stałej dla Siatki
def get_siatki_constant(grammage):
...
with tab1:
    ...
    if st.button("Oblicz", key="calculate_siatki"):
        if speed_siatki <= 0 or stretch <= 0 or grammage_siatki <= 0 or layers_siatki <= 0:
            ...
            constant = get_siatki_constant(grammage_siatki)
            result = (speed_siatki / 100) * (stretch / 100) * (effective_grammage / 1000) * constant
